...
mod generate {
    ...
    // TODO: Why? Keep the DRM format for simplicity...
    ...
    pub fn generate() -> Result<(), Box<dyn Error + Sync + Send>> {
        ...
        let as_enum_path = "src/as_enum.rs";
        {
            ...
            as_enum.write_all(b"use crate::AmdModifier;")?;
            ...
            as_enum.write_all(b"Amd(AmdModifier),")?;
            ...
            as_enum.write_all(
                // b"x if x.vendor() == Ok(Some(DrmVendor::Amd)) => Self::Amd(AmdModifier::from(x)),\n",
                b"x if (x >> 56) as u8 == consts::DRM_FOURCC_AMD => Self::Amd(AmdModifier::from(x)),\n",
            )?;
            ...
            as_enum.write_all(b"Self::Amd(x) => x.into(),\n")?;
